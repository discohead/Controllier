...
@Observable final class ControlSystem {
...
//    let output: OutputModuleProtocol
    ...
    var ticksPerBeat: Int = 24
    
    init() {
        var state = GlobalState()
        self.globalState = state
        self.midiService = MIDIService()
        let noteGenerator = NoteGenerator(op: ops.sine())
        
//        self.oscService = oscService
//        self.output = output
        self.timeline = Timeline(globalState: state)
        self.clock = CallbackClock(tempo: state.tempo, ticksPerBeat: 24)
        self.channels = [Channel(globalState: state, noteGenerator: noteGenerator, number: 0)]
        self.globalState.$tempo
...
//        )
    ...
    private func setupChannels() {
        channels.forEach { channel in
            globalState.controllerState.observe(channel: channel.number.toUInt4, controller: 1)
                .coalesce(for: 0.033)
                .sink { ccVal in
                    channel.noteGenerator.numBeats = ccVal.unitIntervalValue
                }
                .store(in: &cancellables)
        }
    }
...
//    }
    ...
    func scheduleNote(
        ...
        let noteOnAction = ScheduledAction(
            ...
            let noteOn = MIDIEvent.noteOn(note, velocity: .midi1(velocity), channel: channel)
            try? self.midiService.send(noteOn)
        }
        ...
        let noteOffAction = ScheduledAction(
            ...
            let note = action.state["note"] as? UInt7 ?? note
            let channel = action.state["channel"] as? UInt4 ?? channel
            let noteOff = MIDIEvent.noteOff(note, velocity: .midi1(0), channel: channel)
            try? self.midiService.send(noteOff)
        }
    ...
    func scheduleCC(
        ...
        let ccAction = ScheduledAction(
            ...
            let channel = action.state["channel"] as? UInt4 ?? channel
            let ccEvent = MIDIEvent.cc(controller, value: .midi1(value), channel: channel)
            try? self.midiService.send(ccEvent)
        }
